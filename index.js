...
const uniswapFactory = new ethers.Contract(config.uniswapFactoryAddress, dexFactoryABI, wallet);
const uniswapRouter = new ethers.Contract(config.uniswapRouterAddress, dexRouterABI, wallet);
...
async function getTokenPrices(tokenA, tokenB) {
  console.log(`Getting prices for ${chalk.yellow(tokenA)} and ${chalk.yellow(tokenB)}...`);

  const pairAddress = await uniswapFactory.getPair(tokenA, tokenB);
  console.log(`Pair address: ${chalk.blue(pairAddress)}`);

  const pairContract = new ethers.Contract(pairAddress, config.uniswapPairABI, provider);
  const [tokenAReserves, tokenBReserves] = await pairContract.getReserves();

  console.log(`Token A (${chalk.yellow(tokenA)}) reserves: ${chalk.green(ethers.utils.formatEther(tokenAReserves))}`);
  console.log(`Token B (${chalk.yellow(tokenB)}) reserves: ${chalk.green(ethers.utils.formatEther(tokenBReserves))}`);
  ...
  console.log(`Token A (${chalk.yellow(tokenA)}) decimals: ${chalk.blue(tokenADecimals.toString())}`);
  console.log(`Token B (${chalk.yellow(tokenB)}) decimals: ${chalk.blue(tokenBDecimals.toString())}`);

  const tokenAPrice = tokenBReserves.mul(10 ** tokenADecimals).div(tokenAReserves.mul(10 ** tokenBDecimals));
  const tokenBPrice = tokenAReserves.mul(10 ** tokenBDecimals).div(tokenBReserves.mul(10 ** tokenADecimals));

  console.log(`Token A (${chalk.yellow(tokenA)}) price: ${chalk.green(ethers.utils.formatUnits(tokenAPrice, tokenADecimals))}`);
  console.log(`Token B (${chalk.yellow(tokenB)}) price: ${chalk.green(ethers.utils.formatUnits(tokenBPrice, tokenBDecimals))}`);
...
function calculateArbitrageProfit(rates) {
  ...
  for (let i = 0; i < rates.length; i++) {
    ...
    const sellRate = rates.find(rate => rate[0] === sellCurrency && rate[1] === buyCurrency)[2];
    ...
    if (potentialProfit > 0) {
      ...
      console.log(chalk.yellow(`Buy ${buyCurrency} at rate ${formattedBuyRate}`));
      console.log(chalk.yellow(`Sell ${sellCurrency} at rate ${formattedSellRate}`));
  ...
  return profit > 0 ? chalk.green(`Total profit: ${(profit * 100).toFixed(2)}%`) : chalk.red('No arbitrage opportunities found');
}
